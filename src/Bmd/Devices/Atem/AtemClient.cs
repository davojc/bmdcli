...
/// <item>The dump ends at the <b>first packet carrying no blocks</b>. There is no end marker.</item>
/// </list></summary>
public sealed class AtemClient : IAsyncDisposable
{
    ...
    async Task ReceiveLoopAsync(CancellationToken stopping)
    {
        while (!stopping.IsCancellationRequested)
        {
            ...
            if (blocks.Count == 0)
            {
                // A payload-free packet after data has flowed is how the dump signals completion.
                if (_sawData) _dumpComplete.TrySetResult();
                continue;
            }
            _sawData = true;
